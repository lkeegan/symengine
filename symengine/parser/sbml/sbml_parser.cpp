                             ...
                             const RCP<const Basic> &x)
{
    ...
    // (http://model.caltech.edu/software/libsbml/5.18.0/docs/formatted/c-api/libsbml-math.html)
                                         ...
                                         vec_basic &params)
{
    ...
    const static std::map<
        ...
        = {{"sin", sin},        {"cos", cos},       {"tan", tan},
           ...
           {"log", log10}, // todo: can be log10 or ln or illegal
           {"log10", log10},    {"ceil", ceiling},  {"ceiling", ceiling},
           {"factorial", fact}, {"floor", floor}};
    // delay (??): probably ignore this for now..
    ...
    if (lname == "piecewise") {
        ...
        if (params.size() % 2 == 1) {
            // piecewise can also have a default value if conditions are false
...
RCP<const Basic> SbmlParser::parse_identifier(const std::string &expr)
{
    ...
    auto l = local_parser_constants.find(lexpr);
    if (l != local_parser_constants.end()) {
        return l->second;
    }
    auto c = parser_constants.find(lexpr);
